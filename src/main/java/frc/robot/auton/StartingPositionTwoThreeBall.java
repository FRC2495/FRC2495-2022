...
public class StartingPositionTwoThreeBall extends CommandGroup {
	/**
	 * Add your docs here.
	 */
	...
	public StartingPositionTwoThreeBall() {
		...
		// Moves from starting point two to cargo pickup area
		...
		addSequential(new DrivetrainTurnAngleUsingPidControllerWithStallDetection(90));
		// Turns the robot by 90 degrees

		addSequential(new DrivetrainMoveDistanceWithStallDetection(+AutonConstants.DISTANCE_FROM_SHOOTING_ZONE_TO_THIRD_CARGO));
		// Moves from shooting zone to the third cargo

		addSequential(new DrivetrainTurnAngleUsingPidControllerWithStallDetection(-43));
		// Turns the robot by 43 degrees

		addSequential(new DrivetrainTurnAngleFromCameraUsingPidController());
		// Turns the robot towards the hub

		addSequential(new FeederTimedFeed(3));
		// Feeds for max 3 secs
