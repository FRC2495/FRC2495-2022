...
import frc.robot.commands.drivetrain.DrivetrainJoystickControl;
...
public class SparkMaxDrivetrain extends Subsystem implements PIDOutput, PIDOutput2, PIDOutput3, IDrivetrain {
	...
	CANSparkMax masterLeft, masterRight; // motor controllers
	CANSparkMax followerLeft, followerRight; // motor controllers
	...
	public SparkMaxDrivetrain(CANSparkMax masterLeft_in, CANSparkMax masterRight_in , CANSparkMax followerLeft_in, CANSparkMax followerRight_in, PIDSourceADXRS450_Gyro gyro_in, Robot robot_in, ICamera camera_in) 
	{
		...
		followerLeft.restoreFactoryDefaults();
		masterRight.restoreFactoryDefaults();
		followerRight.restoreFactoryDefaults();
		...
		masterLeft.setIdleMode(IdleMode.kCoast); // sets the talons on brake mode
		...
		RelativeEncoder leftEncoder = masterLeft.getEncoder();
		leftEncoder.setInverted(false);
		masterLeft.getPIDController().setFeedbackDevice(leftEncoder);

		RelativeEncoder rightEncoder = masterRight.getEncoder();
		rightEncoder.setInverted(false);
		masterRight.getPIDController().setFeedbackDevice(rightEncoder);
		...
		SparkMaxLimitSwitch leftForwardLimit = masterLeft.getForwardLimitSwitch(SparkMaxLimitSwitch.Type.kNormallyClosed);
		leftForwardLimit.enableLimitSwitch(false);
		SparkMaxLimitSwitch leftReverseLimit = masterLeft.getReverseLimitSwitch(SparkMaxLimitSwitch.Type.kNormallyClosed);
		leftReverseLimit.enableLimitSwitch(false);
		SparkMaxLimitSwitch rightForwardLimit = masterRight.getForwardLimitSwitch(SparkMaxLimitSwitch.Type.kNormallyClosed);
		rightForwardLimit.enableLimitSwitch(false);
		SparkMaxLimitSwitch rightReverseLimit = masterRight.getReverseLimitSwitch(SparkMaxLimitSwitch.Type.kNormallyClosed);
		rightReverseLimit.enableLimitSwitch(false);
	...
	public void moveDistance(double dist, double percentOutput) // True is low gear setting, false is high gear setting
	{
		...
		masterRight.getPIDController().setReference(rtac, ControlType.kPosition);
		masterLeft.getPIDController().setReference(ltac, ControlType.kPosition);
	...
	public void setPIDParameters()
	{
		...
		SparkMaxPIDController rightController = masterRight.getPIDController();
		rightController.setP(MOVE_PROPORTIONAL_GAIN);
		rightController.setI(MOVE_INTEGRAL_GAIN);
		rightController.setD(MOVE_DERIVATIVE_GAIN);

		SparkMaxPIDController leftController = masterLeft.getPIDController();
		leftController.setP(MOVE_PROPORTIONAL_GAIN);
		leftController.setI(MOVE_INTEGRAL_GAIN);
		leftController.setD(MOVE_DERIVATIVE_GAIN);
	...
	public void setNominalAndPeakOutputs(double peakOutput)
	{
		...
		SparkMaxPIDController rightController = masterRight.getPIDController();
		rightController.setOutputRange(-peakOutput, peakOutput);

		SparkMaxPIDController leftController = masterLeft.getPIDController();
		leftController.setOutputRange(-peakOutput, peakOutput);
	}
