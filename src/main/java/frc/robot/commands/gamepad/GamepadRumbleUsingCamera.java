...
public class GamepadRumbleUsingCamera extends Command {
	...
	//static final double VERTICAL_OFFSET_MIN = -20;
	//static final double VERTICAL_OFFSET_MAX = 15;
	static final double THRESHOLD_RPM = 100;
	...
	protected void execute() {
		...
		double real_rpm = Robot.shooter!=null?Robot.shooter.getRpm():3200;


		if (Robot.camera != null && Robot.camera.getNumberOfTargets() > 0 && 
			Math.abs(Robot.camera.getAngleToTurnToCompositeTarget()) < 5 &&
			Math.abs(real_rpm - magic_rpm) < THRESHOLD_RPM)
			//Robot.camera.getFilteredVerticalOffsetToCompositeTarget() > VERTICAL_OFFSET_MIN &&
			//Robot.camera.getFilteredVerticalOffsetToCompositeTarget() < VERTICAL_OFFSET_MAX)
